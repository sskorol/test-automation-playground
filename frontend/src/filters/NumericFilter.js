        ...
        if (list.length > 0) {
            ...
            for (let key in list) {
                ...
                if (obj.indexOf('-') > 0) { // handle dash
                    ...
                    rules.push({ type: RuleType.GreaterThenEqual, value: begin });
                } else if (obj.indexOf('<=') > -1) { // handle less then or equal
                    let end = parseFloat(obj.split('<=')[1]);
                    rules.push({ type: RuleType.LessThenEqual, value: end });
        ...
        return (
            <div>
                <div style={columnStyle}>
                    <input key={inputKey} type="text" placeholder="e.g. 3,10-15,>20"
